...
export class GamesService {
	...
	public async createGame(data: SaveGameDto): Promise<Game> {
		...
		let tiles = getEmptyTiles(userIds.length);
		let randomTiles = [];
		let map = getMatrix(16, 16);
	...
	private openStartingTile(game: Game, playersTablePosition: number): Promise<Game> {
		...
		if(game.playersCount === 2) {
			...
			console.log(playersTablePosition, x, y, coordinateX, coordinateY);
		}
	...
	private async openTile(x: number, y: number, game: Game, coordinates: OpenTileDto): Promise<Game> {
		...
		const tileXPosition = (game.playersCount % 2 === 0 || tileYPosition % 2 !== 0) ? Math.floor(coordinates.x / 4) : Math.floor((coordinates.x - (3 - tileYPosition) * 2) / 4);
		...
		if(x === coordinates.x) {
				...
				for(let i = 0; i < 4; i++) {
					for(let j = 0; j < 4; j++) {
						if(map[i + mapOpeningY][j + mapOpeningX] === null) {
							let cellTemplate = await this.mapService.getCellTemplate({ id: currentTile[3 - i][3 - j] });
							if(cellTemplate.hutMarker || cellTemplate.villageMarker) {
								let loot = await this.getRandomLoot(game.id, cellTemplate.villageMarker);
								let cell = await this.mapService.saveCell({ cellId: cellTemplate.id, lootId: loot.id, gameId: game.id, x: j + mapOpeningX, y: i + mapOpeningY });
								map[i + mapOpeningY][j + mapOpeningX] = cell.id;
							}
							else {
								let cell = await this.mapService.saveCell({ cellId: cellTemplate.id, relict: cellTemplate.relict, gameId: game.id, x: j + mapOpeningX, y: i + mapOpeningY });
								map[i + mapOpeningY][j + mapOpeningX] = cell.id;
							}
						}
					}
			...
			if(y > coordinates.y) {
				for(let i = 0; i < 4; i++) {
					for(let j = 0; j < 4; j++) {
						if(map[i + mapOpeningY][j + mapOpeningX] === null) {
							let cellTemplate = await this.mapService.getCellTemplate({ id: currentTile[i][j] });
							if(cellTemplate.hutMarker || cellTemplate.villageMarker) {
								let loot = await this.getRandomLoot(game.id, cellTemplate.villageMarker);
								let cell = await this.mapService.saveCell({ cellId: cellTemplate.id, lootId: loot.id, gameId: game.id, x: j + mapOpeningX, y: i + mapOpeningY });
								map[i + mapOpeningY][j + mapOpeningX] = cell.id;
		...
		if(y === coordinates.y) {
			if(x < coordinates.x) {
				for(let i = 0; i < 4; i++) {
					for(let j = 0; j < 4; j++) {
						if(map[i + mapOpeningY][j + mapOpeningX] === null) {
							let cellTemplate = await this.mapService.getCellTemplate({ id: currentTile[3 - j][i] });
							if(cellTemplate.hutMarker || cellTemplate.villageMarker) {
								let loot = await this.getRandomLoot(game.id, cellTemplate.villageMarker);
								let cell = await this.mapService.saveCell({ cellId: cellTemplate.id, lootId: loot.id, gameId: game.id, x: j + mapOpeningX, y: i + mapOpeningY });
								map[i + mapOpeningY][j + mapOpeningX] = cell.id;
			...
			if(x > coordinates.x) {
				for(let i = 0; i < 4; i++) {
					for(let j = 0; j < 4; j++) {
						if(map[i + mapOpeningY][j + mapOpeningX] === null) {
							let cellTemplate = await this.mapService.getCellTemplate({ id: currentTile[j][3 - i] });
							if(cellTemplate.hutMarker || cellTemplate.villageMarker) {
								let loot = await this.getRandomLoot(game.id, cellTemplate.villageMarker);
								let cell = await this.mapService.saveCell({ cellId: cellTemplate.id, lootId: loot.id, gameId: game.id, x: j + mapOpeningX, y: i + mapOpeningY });
								map[i + mapOpeningY][j + mapOpeningX] = cell.id;
