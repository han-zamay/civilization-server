...
export class MovementService {
	...
	public async figureMove(data: FigureMoveDto): Promise<figureMoveResponse> {
		...
		await this.mapService.saveCell({
			...
			playerId: remainingFigures.length ? startingCell.player?.id : null,
		});
		await this.mapService.saveCell({ id: targetPoint.id, figures: isPathCorrect.isFigureDisbanding ? null : figures });

		const movedFigures = await Promise.all(
			isPathCorrect.figures.map((figure) =>
				this.playersService.savePlayersFigure({
					id: figure.id,
		...
		if (isPathCorrect.isFigureDisbanding) {
			const city = await this.citiesService.save({
				id: targetPoint.city.id,
				fortificationMarker: figures[0].isArmy,
				caravanMarker: !figures[0].isArmy,
			});
	...
	public async isPathCorrect(data: FigureMoveDto): Promise<isPathCorrectResponse> {
		...
		const isFigureDisbanding = targetPoint.player !== null && targetPoint.player?.id === player.id && player.cities.includes(targetPoint.city);
		...
		if (
			...
			targetPoint.player?.id === player.id &&
			targetPoint.figures.length + figures.length > player.stakingLimit
		) {
		...
		for (let i = 1; i < cells.length; i++) {
			const areAdjust = areCellsAdjust(cells[i - 1].x, cells[i - 1].y, cells[i].x, cells[i].y);
			if (!areAdjust) {
