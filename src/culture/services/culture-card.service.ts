...
import { CultureCardCounts, CultureCardLevels, CultureTrack } from '../culture-consts';
...
export class CultureCardService {
    ...
    public async prepareCultureCards(gameId: number): Promise<CardInGame[]> {
        ...
        const culturePile = this.cultureCardRepository.saveMany(allCards.map((card) => ({
            name: card,
            gameId,
            playerId: null,
        })));
    ...
    private async gainCultureCard(player: Player, level: number): Promise<CardInGame> {
        ...
        if(properLevelCards.length === 0) {
            ...
            await this.cultureCardRepository.saveMany(properLevelCards.map((card) => ({
                id: card.id,
                isUsed: false,
            })));
        }
